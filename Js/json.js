...
const buildData = () => {
  ...
  for (let i=0;i<allPhoto.length-4000;i++){ //LIST of PhotoData
    const photoData_ith = allPhoto[i];
    const albumNameById = allAlbum.filter(function (currentElement) { //ARRAY of Album
      return currentElement.id === photoData_ith.albumId;
    });
    const userNameById = allData.filter(function (currentElement) { //ARRAY of User
...
const renderTable = () => {
    const datas = User.slice(currentIndex * limit, (currentIndex + 1) * limit);
    ...
    datas.forEach((data, idx) => {
      tr += `
              <tr>
                  ...
                  <td>${data.user}</td>
                  <td><img id="myImg" src="${data.thumbnail}" onClick="imgPop('${data.url}')"></td>
                  <td></td>
                  <td></td>
              </tr>
          `;
  ...
  const renderPageButton = () => {
    ...
    const pageTotal = User.length / limit;
  ...
  const nextPage = () => {
    ...
    if (User.length / limit > currentIndex + 1) {
      currentIndex += 1;
      renderTable();
    }
  };
